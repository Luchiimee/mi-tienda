...
export default function ProductosPage() {
  ...
  // --- SOLUCIÓN 1: INICIALIZACIÓN ROBUSTA ---
  const handleOpenEditor = (product?: Product) => {
      if (product) {
          // Detectamos si hay galería, si no, usamos la imagen legacy, si no, array vacío
          ...
          setFormData({ 
              ...product, 
              galeria: safeGallery,
              // Aseguramos que imagen tenga valor si existe en la galería
  ...
  const productosVisibles = (shopData.productos as Product[]).filter(p => p.tipo === tipoActual);

  return (
    <div className="contenedor-layout">
      <Sidebar activeTab="productos" />

      <main className="main-content" style={{ padding: '40px', background: '#f8fafc', minHeight: '100vh', display:'flex', flexDirection:'column', justifyContent:'flex-start' }}>
        
        <div style={{ marginBottom: 30 }}>
            <h1 style={{ margin: 0, color: '#1e293b', fontSize: 28, fontWeight: '800' }}>Mis Productos</h1>
            <p style={{ margin: '5px 0 0', color: '#64748b', fontSize: 14, textAlign:'center' }}>Gestionando: <b>{shopData.template.toUpperCase()}</b></p>
        </div>
                    ...
                    <div key={p.id} style={{ background: 'white', borderRadius: 12, overflow: 'hidden', border: '1px solid #e2e8f0', boxShadow: '0 2px 5px rgba(0,0,0,0.02)', transition: 'transform 0.2s', display:'flex', flexDirection:'column' }}>
                        ...
                        {p.galeria && p.galeria.length > 1 && (
                            <div style={{ display: 'flex', gap: 4, padding: '8px 12px 0 12px', overflowX:'hidden' }}>
                                {p.galeria.slice(1, 4).map((img: string | Blob | undefined, i: Key | null | undefined) => (
        ...
        {/* --- MODO EDITOR --- */}
        {(isCreating || editingId) && (
            // SOLUCIÓN 2: EL KEY MÁGICO
            // Usamos key={editingId || 'new'} para forzar a React a destruir el formulario viejo 
            // y crear uno nuevo. Adiós a las imágenes fantasma o retrasos.
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    ...
                    <div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 15, marginBottom: 15 }}>
                            {(() => {
                                // Doble chequeo de seguridad para que nunca explote
