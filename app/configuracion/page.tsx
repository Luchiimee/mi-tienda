...
export default function ConfiguracionPage() {
  ...
  const handleLock = async (tmpl: string) => {
    if (!tmpl) return;
  ...
  return (
    <div className="contenedor-layout">
      ...
      <main className="main-content" style={{ padding: '40px', overflowY: 'auto', background: '#f8fafc', width: '100%', minHeight: '100vh', justifyContent:'start'}}>
        ...
        {/* CONTENEDOR GRID: AJUSTADO PARA QUE ENTREN LADO A LADO */}
        <div style={{ 
            display: 'grid', 
            // CAMBIO CLAVE: Bajé de 350px a 280px para que entren las 3 en fila
            ...
            <div style={{ background: 'white', padding: 30, borderRadius: 16, boxShadow: '0 4px 20px rgba(0,0,0,0.03)', border:'1px solid #f1f5f9' }}>
                ...
                <div style={{ border:'1px dashed #cbd5e1', padding: 20, borderRadius: 12 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                        ...
                        <button onClick={handlePassChange} disabled={loadingPass} style={{ padding: '0 20px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 'bold', fontSize:12 }}>
                            {loadingPass ? '...' : 'OK'}
                        </button>
                    </div>
                </div>
            ...
            <div style={{ background: 'white', padding: 30, borderRadius: 16, boxShadow: '0 4px 20px rgba(0,0,0,0.03)', border:'1px solid #f1f5f9' }}>
                ...
                <div style={{display:'flex', gap:15, marginBottom: 25}}>
                    <button 
                        onClick={() => updateProfile({ plan: 'simple' })}
                        style={{ flex: 1, padding: 12, border: isSimple ? '2px solid #94a3b8' : '1px solid #e2e8f0', borderRadius: 8, background: 'white', cursor: 'pointer', color: '#64748b', fontWeight:'bold', fontSize:13 }}
                    >
                        SIMPLE
                    </button>
                    <button 
                        onClick={() => updateProfile({ plan: 'full' })}
                        style={{ flex: 1, padding: 12, border: !isSimple ? '2px solid #eab308' : '1px solid #e2e8f0', borderRadius: 8, background: 'white', cursor: 'pointer', color: '#ca8a04', fontWeight:'bold', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}
                    >
                        FULL 👑
                    </button>
                </div>
                ...
                {!isSimple ? (
                    <div style={{ fontSize: 13, color: '#166534', background:'#dcfce7', padding:15, borderRadius:8, textAlign:'center', border:'1px solid #bbf7d0' }}>
                        ✨ ¡Tienes libertad total de plantillas!
                    </div>
                ) : (
                    <div style={{ background: '#fff7ed', padding: 20, borderRadius: 8, border: '1px solid #fed7aa' }}>
                        ...
                        <select 
                            ...
                            style={{ width: '100%', padding: 10, borderRadius: 6, border: '1px solid #fdba74', color: '#9a3412', fontWeight: 'bold', cursor: 'pointer', background:'white' }}
                        >
            ...
            <div style={{ background: 'white', padding: 30, borderRadius: 16, boxShadow: '0 4px 20px rgba(0,0,0,0.03)', border:'1px solid #f1f5f9' }}>
                ...
                {/* TARJETA VISUAL */}
                <div style={{
                    background: 'linear-gradient(135deg, #4b89dc 0%, #2e5b98 100%)', 
                    padding: 25, borderRadius: 12, color: 'white', marginBottom: 25,
                    boxShadow: '0 8px 20px rgba(46, 91, 152, 0.25)', position:'relative', overflow:'hidden'
                }}>
                    <div style={{fontSize:18, fontWeight:'bold', marginBottom:25, display:'flex', alignItems:'center', gap:8}}>
                        Mercado Pago <span style={{fontSize:10, background:'rgba(255,255,255,0.2)', padding:'3px 6px', borderRadius:4, fontWeight:'normal'}}>PRO</span>
                    </div>
                    ...
                    <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-end'}}>
                        <div>
                            <div style={{fontSize:9, opacity:0.8, marginBottom:2}}>TITULAR</div>
                            <div style={{fontSize:12, fontWeight:'bold', textTransform:'uppercase'}}>{shopData.nombreDueno || 'USUARIO'} {shopData.apellidoDueno}</div>
                        </div>
                        <div style={{textAlign:'right'}}>
                            <div style={{fontSize:9, opacity:0.8, marginBottom:2}}>EXP</div>
                            <div style={{fontSize:12, fontWeight:'bold'}}>12/28</div>
                        </div>
                    </div>
                </div>
                ...
                <div style={{textAlign:'center'}}>
                    <p style={{fontSize:12, color:'#64748b', marginBottom:15}}>
                        Gestiona tu método de pago para la suscripción mensual del Plan Full.
                    </p>
                    <button disabled style={{width:'100%', padding:12, background:'#f1f5f9', color:'#94a3b8', border:'none', borderRadius:8, cursor:'not-allowed', fontWeight:'bold', fontSize:13}}>
                        Configurar Pagos (Próximamente)
                    </button>
                </div>
            </div>
