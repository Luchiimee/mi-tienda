...
export const ShopProvider = ({ children }: { children: ReactNode }) => {
  ...
  const isSuperAdmin = (shopData.email === ADMIN_EMAIL) || (sessionEmail === ADMIN_EMAIL);
  ...
  const loadShopData = useCallback(async (userParam?: any) => {
    try {
        ...
        // --- FIX ANTI-DUPLICADOS ---
        // 1. Buscamos TODAS las tiendas del usuario
        let { data: shops } = await supabase.from('shops')
            .select('*')
            .eq('owner_id', user.id)
        ...
        // 2. Si existe AL MENOS UNA, usamos la primera (la más reciente)
        if (shops && shops.length > 0) {
            shop = shops[0];
            // Opcional: Si hay más de una, podríamos borrar las viejas aquí, pero mejor no tocar por seguridad.
            console.log(`✅ Tienda encontrada: ${shop.id} (Total encontradas: ${shops.length})`);
        } else {
            // 3. SOLO creamos si la lista está VACÍA
             console.log("🆕 No se encontraron tiendas. Creando la primera...");
        ...
        if (shop) {
          ...
          if (dbItems && dbItems.length > 0) items = dbItems;
              ...
              items = insertError ? defaults.map((p, i) => ({ ...p, id: `local-${i}`, shop_id: shop.id })) : inserted;
          }
          ...
          setShopData({
            id: shop.id, email: userEmail, nombreAdmin: nombreFinal || (userEmail.split('@')[0]), 
  ...
  const activateTrial = async (selectedPlan: 'simple' | 'full', selectedTemplate?: string) => {
      if (!shopData.id) return alert("Error: No se encontró la tienda.");
      
      const now = new Date().toISOString();
      let nextCount = shopData.changeCount || 0;
      
      // 1. Verificamos si estamos intentando CAMBIAR de plantilla en el plan básico
      ...
      const updates: any = { plan: selectedPlan, subscription_status: 'trial', trial_start_date: shopData.trial_start_date || now };
      ...
      setShopData(prev => {
          ...
          return { 
              ...
              subscription_status: updatedShop.subscription_status, 
              trial_start_date: updatedShop.trial_start_date, 
              template: updatedShop.template, 
              templateLocked: updatedShop.template_locked,
              lastTemplateChange: updatedShop.last_template_change, 
              changeCount: updatedShop.change_count || 0,
              slugs: currentSlugs, 
              slug: currentSlugs[updatedShop.template] || prev.slug
          };
      });

      if (selectedPlan === 'simple' && selectedTemplate && selectedTemplate !== shopData.template) {
           await refreshProductsForTemplate(selectedTemplate, shopData.id); 
      }
  ...
  const changeTemplate = async (newTemplate: string) => {
    ...
    await refreshProductsForTemplate(newTemplate, shopData.id!); 
    return { success: true };
  };

  const refreshProductsForTemplate = async (tmpl: string, shopId: string) => {
       const tipo = getTipo(tmpl);
       let items: any[] = [];
       const { data } = await supabase.from('products').select('*').eq('shop_id', shopId).eq('tipo', tipo).order('created_at', { ascending: true });
       
       if (!data || data.length === 0) {
          const defs = getDefaults(tmpl).map((p, i) => ({ ...p, shop_id: shopId, imagen_url: (p as any).imagen_url }));
          const { data: ins } = await supabase.from('products').insert(defs).select();
          items = ins || defs.map((p, i) => ({...p, id: `loc-${i}`}));
       } else items = data;
       
       setShopData(prev => {
           const nuevoNombre = prev.nombres[tmpl] || prev.nombreNegocio;
           const nuevaDesc = prev.descripciones[tmpl] || prev.descripcion;
           const nuevoLogo = prev.logos[tmpl] || prev.logo;
           const nuevoSlug = prev.slugs[tmpl] || '';

           return { 
               ...prev, 
               template: tmpl, 
               nombreNegocio: nuevoNombre,
  ...
  const resetTemplate = async (tmplToReset: string) => { 
      ...
      setShopData(prev => ({ ...prev, slugs: { ...prev.slugs, [tmplToReset]: '' } })); 
      const dbData: any = {}; 
  ...
  const updateProfile = async (data: any) => { 
      ...
      if(shopData.id) { 
          const dbData: any = {}; 
          if(data.nombreDueno) dbData.nombre_dueno = data.nombreDueno; 
          if(data.apellidoDueno) dbData.apellido_dueno = data.apellidoDueno; 
          if(data.telefonoDueno) dbData.telefono_dueno = data.telefonoDueno; 
          if(shopData.email) dbData.email = shopData.email; 
          if(data.plan) { 
              dbData.plan = data.plan; 
              if(data.plan === 'full') dbData.template_locked = null; 
          } 
          await supabase.from('shops').update(dbData).eq('id', shopData.id); 
      } 
  };
  ...
  const addProduct = async (prod: Product) => { 
      ...
      const { data } = await supabase.from('products').insert([{ shop_id: shopData.id, titulo: prod.titulo, descripcion: prod.descripcion, precio: prod.precio, galeria: prod.galeria || [], url_destino: prod.url, tipo: tipoActual, imagen_url: prod.imagen }]).select().single(); 
      if (data) { 
          setShopData((prev:any) => ({ ...prev, productos: [...prev.productos, { id: data.id, titulo: data.titulo, descripcion: data.descripcion, precio: data.precio, galeria: data.galeria || [], url: data.url_destino, shop_id: data.shop_id, tipo: data.tipo, imagen: data.imagen_url }] })); 
      } 
  };
  ...
  const deleteProduct = async (id: string) => { 
      if (!canEdit()) return; 
      setShopData((prev:any) => ({ ...prev, productos: prev.productos.filter((p:any) => p.id !== id) })); 
